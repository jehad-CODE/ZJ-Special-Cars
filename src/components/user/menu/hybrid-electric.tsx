...
const HybridElectric: React.FC = () => {
  ...
  const carsPerPage = 9;
  const hybridElectricCars = cars.filter(car => car.type === 'Hybrid/Electric');
  ...
  const sortedHybridElectricCars = sortCars(hybridElectricCars);
  const currentCars = sortedHybridElectricCars.slice(startIndex, startIndex + carsPerPage);
  ...
  return (
    <Box sx={{
      ...
      <Box sx={{ 
        ...
        ) : hybridElectricCars.length === 0 ? (
          <Typography variant="h6" sx={{ textAlign: 'center' }}>No hybrid/electric cars available at the moment.</Typography>
        ) : (
          <>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center', pb: 4 }}>
              <Pagination
                count={Math.ceil(hybridElectricCars.length / carsPerPage)}
                page={page}
