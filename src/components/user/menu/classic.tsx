...
const Classic: React.FC = () => {
  ...
  const carsPerPage = 9;
  const classicCars = cars.filter(car => car.type === 'Classic');
  ...
  const sortedClassicCars = sortCars(classicCars);
  const currentCars = sortedClassicCars.slice(startIndex, startIndex + carsPerPage);
  ...
  return (
    <Box sx={{
      ...
      {loading ? (
        ...
        <Typography variant="h6" sx={{ textAlign: 'center' }}>No classic cars available at the moment.</Typography>
      ) : (
        <>
